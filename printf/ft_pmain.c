...
/*   Updated: 2023/11/23 16:36:37 by sabyun           ###   ########.fr       */
/*                                                                            */
/* ************************************************************************** */
...
int	get_string(const char *s, int *size, t_str *list)
{
	...
	list->send = ft_strjoin(list, temp, *size);
	*size = 0;
	if (!list->send)
...
int	check_option(const char *s, t_str *list, va_list ap)
{
	...
	if (*s == 'c')
		...
		temp = ft_strdup(va_arg(ap, char *), list);
	else if (*s == 'p')
	...
	if (!temp)
		return (free_nul(0, "ss", list->send, temp));
	list->send = ft_strjoin(list, temp, list->t_len);
	if (!list->send)
